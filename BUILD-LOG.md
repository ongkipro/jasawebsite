...
## 2026-09-11 — Copy and folio UX refinement

- **Writing:** replaced combative competitor comparisons, unsupported performance language, repeated SOW/guarantee wording, and developer-facing schema labels with direct Indonesian that explains scope and next steps. Prices, live-project count, SEO titles, and verifiable delivery terms were preserved.
- **UX:** restored readable serif H1 hierarchy for portfolio and industry directory; made every portfolio row a native keyboard-operable button; removed its nested scroll region so the parent sheet owns mobile scrolling; and made mobile/tablet subpage labels follow the active folio rather than always saying “Pricing & SOW”.
- **Review and validation:** independent visual review by `/root/designer` identified the portfolio keyboard and mobile clipping defects. `npx tsc --noEmit`, `npm test` (1,634 output assertions plus development-map check), and `npm run test:ui` (83 browser cases and interactions) passed after the fixes.
